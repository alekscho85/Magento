...
class Trackingmore_Detrack_Model_EnableTracking extends Mage_Core_Model_Config_Data
{
    ...
    {     
        ...
        $oldPluginStatus = Mage::getStoreConfig('tr_section_setttings/settings/status');
        Mage::app()->getStore()->setConfig('tr_section_setttings/settings/api_key', $apiKey);
		$oldCrossSellStatus = Mage::getStoreConfig('tr_section_setttings/crosssell/cross_sell_page');
		$crossSellStatus = $this->_data['groups']['crosssell']['fields']['cross_sell_page']['value'];
        ...
        if(!$pluginStatus AND $oldApiKey !== $apiKey){
			...
			$flag = 1;
        }else if(($pluginStatus AND $oldApiKey !== $apiKey) OR ($pluginStatus AND !$oldPluginStatus)){
            $info = $helper->testApiKey($pluginStatus,'GET',$body); 
			$flag = 1;
